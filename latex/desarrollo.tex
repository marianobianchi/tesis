...
En la Figura \ref{fig:esquema} se muestra este esquema de manera gráfica. Las flechas negras indican las transiciones del esquema presentado, y junto a cada transición figuran los datos que se pasan de una etapa a la otra. Las flechas punteadas son transiciones opcionales que indican que el entrenamiento además puede nutrirse de la información recolectada del objeto durante la detección y el seguimiento.

\begin{figure}
	\centering
	\includegraphics[scale=0.5]{img/esquema_seguimiento.png}
	\caption{Gráfico del esquema de seguimiento}
	\label{fig:esquema}
\end{figure}
...
En esta sección explicaremos como se implementó el sistema de seguimiento para secuencias de imágenes RGB basándonos en las etapas explicadas previamente. En el entrenamiento, se toman templates del objeto a seguir tratando de cubrir las distintas caras del objeto. En la detección, estos templates se utilizan para aplicar un método de detección por templates llamado \textit{template matching} \cite{brunelli2009template}. Por último, en la etapa de seguimiento utilizamos técnicas de comparación de histogramas para seguir al objeto frame a frame.
...
En la Figura \ref{fig:template_matching} se muestra una imagen utilizada de template o modelo del objeto y el resultado de aplicar este algoritmo sobre una imagen donde aparece ese objeto.

\begin{figure}
	\centering
	\begin{subfigure}[b]{0.3\textwidth}
		\includegraphics[width=\textwidth]{img/template_matching/template.png}
		\caption{Template}
	\end{subfigure}
	\quad
	\begin{subfigure}[b]{0.5\textwidth}
		\includegraphics[width=\textwidth]{img/template_matching/scene.png}
		\caption{Detección marcada en la escena}
	\end{subfigure}
	\caption{Resultado del algoritmo template matching}
...
La información recolectada durante el entrenamiento se toma con la intención de poder aplicar como método de detección al algoritmo de \textit{Template matching}. La implementación utilizada es la que se encuentra presente en la librería \textit{OpenCV} para C++ \cite{website:opencv}. Esta implementación recibe varios parámetros. El primero es el frame de la escena a donde se desea detectar el objeto. El segundo es el template con el que se va a buscar el objeto. El tercero es opcional y es una máscara para el template del objeto. El último parámetro le indica al algoritmo de template matching que comparación usar para emparejar el template a la imagen. En este trabajo se decide utilizar diferencia cuadrática píxel por píxel normalizada, presente en la fórmula \ref{eq:diferencia_cuadratica}
...
Entre las condiciones de corte para este algoritmo hay un límite máximo para la cantidad de iteraciones, el que se encuentra en el pseudocódigo que corta en caso de que dos transformaciones sucesivas difieran en el error cuadrático en menos de un umbral dado y por último una condición parecida que verifica que el error cuadrático de una transformación obtenida sea menor a un umbral. En la Figura \ref{fig:ejemplo_icp} se pueden ver dos imágenes, una antes y otra después de haber corrido ICP para alinear las dos nubes de puntos.


\begin{figure}[ht]
	\centering
	\begin{subfigure}[b]{0.4\textwidth}
		\includegraphics[width=\textwidth]{img/icp_ini.png}
		\caption{Antes de alinear}
	\end{subfigure}
	\begin{subfigure}[b]{0.4\textwidth}
		\includegraphics[width=\textwidth]{img/icp_fin.png}
		\caption{Después de alinear}
	\end{subfigure}

	\caption{Ejemplo de una alineación usando ICP}
	\label{fig:ejemplo_icp}
\end{figure}
...
El pseudocódigo de este algoritmo puede verse en el Método \ref{lst:alignment_prerejective}.

\begin{algorithm}[h]
\caption{Pseudocódigo de la heurística de alineación de objetos rígidos}
\label{lst:alignment_prerejective}
\begin{algorithmic}
...
Un \kdt \cite{samet2006foundations} es una estructura de datos que permite organizar puntos en un espacio k-dimensional. En particular es un árbol binario con otras restricciones. Son muy útiles para realizar búsquedas por rango o por vecinos cercanos. Una búsqueda por rango es una búsqueda para obtener todos aquellos puntos del árbol que estén contenidos en un rango determinado para una dimensión determinada, permitiendo combinar hasta k rangos, uno por cada dimensión. La búsqueda por vecinos cercanos tiene como objetivo encontrar el punto del árbol más cercano a un punto recibido como parámetro. Existen implementaciones en las cuales esta búsqueda es logarítmica en la cantidad de puntos del árbol. En la Figura \ref{fig:kdtree} se puede observar un \kdt para puntos en dos dimensiones. Notar que cada nodo cumple la misma propiedad que un nodo de un árbol binario de búsqueda (ABB) de que los nodos del subárbol derecho son mayores y los del izquierdo menores, salvo que cambia sutílmente el sentido de mayor o menor. Dependiendo del nivel donde se encuentre el nodo es por la coordenada en que se los compara. En el gráfico se muestra para cada nivel con que coordenada se ordenan los subárboles.

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/kdtree.png}
	\caption{\kdt resultante del conjunto de puntos (2,3), (5,4), (9,6), (4,7), (8,1), (7,2).}
    \label{fig:kdtree}
\end{figure}
...
El método de seguimiento elegido para profundidad es ICP, explicado en la sección \ref{ICP}. En esta sección explicaremos cuales fueron los parámetros explorados para este método y como lo utilizamos dentro del esquema de seguimiento propuesto.
...
Además, una vez que converge ICP la librería facilita el valor de la suma del cuadrado de las distancias de la nube de puntos inicial a la nube de puntos destino como medida de que tan buena es la alineación obtenida. Este valor se utiliza como umbral para decidir si la respuesta encontrada se considera correcta o no, por lo que también se exploran distintos valores para este umbral, como con el resto de los parámetros, para encontrar el que mejor se adapte a nuestras necesidades.

Esta etapa entonces consta de los siguientes pasos:
\begin{itemize}
	\item Tomar la nube reportada por la detección o seguimiento en el frame anterior
	\item Filtrar de la nube del frame actual los puntos contenidos en un cubo el triple de mayor que la ``bounding box'' que contiene a la nube del paso anterior, cuyo centro es el mismo que el de la ``bounding box''
	\item Alinear con ICP estas dos nubes
	\item Si la alineación falla, volver a la etapa de detección
	\item Si es exitosa, se filtran los puntos de la escena que corresponden al objeto al igual que se explicó en la Sección \ref{deteccion_d} y si los puntos superan un porcentaje de la cantidad de puntos que tiene el modelo, se considera encontrado el objeto y se reporta esta ubicación como resultado. Si los puntos obtenidos de la escena son menos de los esperados, se vuelve a la etapa de detección
\end{itemize}



\section{Método propuesto en RGB-D}\label{metodo_rgbd}
En esta sección presentamos el sistema de seguimiento de RGB y de profundidad combinados. Explicaremos de que manera unimos los métodos explicados en las secciones \ref{metodo_rgb} y \ref{metodo_d}. El entrenamiento es una unión directa de los entrenamientos RGB y de profundidad. Para la detección utilizamos \textit{template matching} como primera aproximación y para detectar el objeto en la nube de puntos usamos el mismo algoritmo de detección explicado en la Sección \ref{deteccion_d} previo filtrado de puntos valiéndonos de los datos obtenidos por la detección RGB. Para el seguimiento utilizamos ICP y mejoramos el resultado usando el seguimiento basado en histogramas para RGB.
...
\section{Objetos}
La base cuenta con múltiples vistas de un conjunto de objetos. Estos objetos están organizados en distintas categorías de manera jerárquica tomando como referencia las relaciones de hiperónimo/hipónimo de WordNet \cite{wordnet}. En total hay unas 51 categorías. Cada objeto pertenece a una única categoría o clase de objetos y hay entre 3 y 14 instancias de un mismo objeto por cada categoría. Por ejemplo, en la categoría ``taza'' existen 8 instancias diferentes que se corresponden simplemente a distintas tazas ya sea por forma o por color. En total la base cuenta con secuencias de video RGB-D de 300 objetos de uso diario que junto con los distintos ángulos en los que fueron tomadas las escenas forman un total de 250.000 imágenes RGB-D.
