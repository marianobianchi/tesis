...
\begin{itemize}
	...
	\item Porcentaje de falsos negativos: cantidad de veces que el algoritmo no encuentra el objeto cuando en realidad el mismo está en la imagen o que lo encuentra pero el área reportada se solapa en menos de un 30\%\footnote{En la Sección \ref{sec:evaluacion_rgbd} se explica a través de la Figura \ref{fig:accuracy_sistema} por qué se elige este valor.} con el área reportada por el ground truth
\end{itemize}
...
Finalmente, existe un color más que puede aparecer: el negro. El método descripto en la Sección \ref{alignment_prerejective} utiliza RANSAC como esquema para obtener la estimación de pose. Una de las características de este tipo de esquemas es que la exploración se realiza de manera aleatoria. Esto provoca que dos corridas del algoritmo con mismos valores de entrada puedan tener distintos valores de salida. Para corroborar la robustes de nuestro algoritmo se ejecuta varias veces para cada valor de entrada y luego al momento de hacer este gráfico comparamos los valores que obtuvimos en cada corrida para cada frame. Si en todas las corridas se obtuvo el mismo resultado, ese frame tiene un punto de alguno de los colores antes mencionados a la altura que corresponda. Si en cambio hay distintos resultados entre las corridas, ese frame tendrá un punto de color negro y la altura del punto será el promedio del área solapada entre todas las corridas para ese frame.
